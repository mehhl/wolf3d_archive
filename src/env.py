...
class ActionRepeatWrapper(dm_env.Environment):
 ...
 def step(self, action):
  reward = 0.0
  discount = 1.0
  # ugly fix to preserve reward shape if its an array
  _count_rewards = self._env._env.reward_spec().shape[0]
  rewards = np.zeros(_count_rewards, dtype=np.float64)
  for i in range(self._num_repeats):
   for r in range(_count_rewards):
    time_step = self._env.step(action)
    rewards[r] += (time_step.reward[r] or 0.0) * discount
    discount *= time_step.discount
    if time_step.last():
     break

  return time_step._replace(reward=rewards, discount=discount)
