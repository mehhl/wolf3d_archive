...
def train():
    ...
    s_cfg.episode_length = 10
    l_cfg.episode_length = 10
    l_cfg.obs_shape = [223]
    l_cfg.action_dim = 38
    s_cfg.obs_shape = [78]
    s_cfg.action_dim = 12
    ...
    learner.load(Path().cwd() / '..' / 'weights' / f'final_learner_model.pt')
    ...
    for step in range(0, 
        ...
        obs = env.reset()
        _s_action_range = lambda i: np.arange(225+78*i, 225+78*(i+1))
        l_episode = Episode(l_cfg, obs[0:223])
        ...
        while i < l_cfg.episode_length:
            ...
            l_episode += (obs[0:223], l_action, reward[0], done)
            print(reward[0])
            i += 1
