...
def list_texcoords(fp):
    """List texcoords from an .obj stored in path fp."""
    texcoords = []
    with open(fp, 'r') as f:
        for line in f:
            parts = line.split()
...
texcoords = list_texcoords(obj_fp)
texcoords = texcoords[0:len(src_vertices)]
...
target_skn = skin.Skin(src_vertices, texcoords, src_faces, bones)
